...
for each ligand color. RGB values are normalized by the clear channel for
lighting resilience while preserving color information.
...
def calibrate():
    ...
    # Per-sensor calibration: sensors[ch][color_name] = {r, g, b}
    ...
    for color_name in LIGAND_COLORS:
        ...
        for ch in range(NUM_SENSORS):
            ...
            sensor_maps[ch][color_name] = {"r": nr, "g": ng, "b": nb}
    ...
    for ch in range(NUM_SENSORS):
        ...
        avg_r, avg_g, avg_b, avg_c = avg
        nr, ng, nb = service.normalize_by_clear(avg_r, avg_g, avg_b, avg_c)
        sensor_maps[ch]["None"] = {"r": nr, "g": ng, "b": nb}

        if prox is not None:
            empty_proxs[ch] = prox

        print(f" done. R/C G/C B/C: ({nr}, {ng}, {nb})  Prox: {prox}")
    ...
    # Save
    color_map = {
        "_comment": "Per-sensor clear-normalized RGB values. Generated by color_calibration.py.",
    ...
    for ch in range(NUM_SENSORS):
        ...
        for name, vals in sensor_maps[ch].items():
            print(f"    {name}: R/C={vals['r']}, G/C={vals['g']}, B/C={vals['b']}")
