...
def read_sensor_avg(service, channel, num_samples=10, delay=0.1):
    """Take multiple readings from a single sensor and return the average."""
    readings = []
...
def calibrate():
    ...
    for color_name in LIGAND_COLORS:
        ...
        for ch in range(NUM_SENSORS):
            ...
            print(f" done. Normalized: ({nr}, {ng}, {nb})")

    # Calibrate "None" (empty slot) per sensor
    ...
    # Save
    color_map = {
        "_comment": "Per-sensor calibrated RGB reference values. Generated by color_calibration.py.",
